<div>
    ...
    <section class="w-full pt-10 pb-6 px-6">
        <h1 class="block text-2xl font-bold mb-2">Create Page</h1>
        <div class="flex items-center mb-4">
            <a href="/admin/dashboard" wire:navigate class="text-sm font-medium text-gray-500 hover:text-gray-700">
                Tableau de board
            </a>
            ...
            <a href="/admin/pages" class="text-sm font-medium text-gray-500 hover:text-gray-700">
                {{ __('Pages') }}
            </a>
            <svg class="inline-block w-3 h-3 mx-3 text-gray-500 fill-current" xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 320 512">
        ...
        <div class="flex flex-wrap">
            @if (session()->has('message'))
                <div class="flex justify-between w-full px-4 py-2 mb-4 text-white bg-green-500 rounded-md">
                    <p>{{ session('message') }}</p>
                    <a href="/admin/page/create" wire:navigate class="text-sm font-bold text-white">Create New
                        Page</a>
                </div>
            @endif
            ...
            <div class="w-3/4 bg-white rounded-lg py-4 px-4">
                <div class="flex flex-col mb-4">
                    ...
                    <x-input-error :messages="$errors->get('form.title')" for="" class="mt-2" />
                </div>
                        ...
                        <div class="flex flex-col space-y-2 py-4">
                            @foreach ($form->cardContent as $parentIndex => $item)
                                <div class="border p-2" wire:key="cardItems-{{ $parentIndex }}">
                                    <label for="cardTitle{{ $parentIndex }}" class="block font-medium mb-2">Titre
                                        de
                                        section:</label>
                                    <input type="text" id="cardTitle{{ $parentIndex }}"
                                        wire:model="cardContent.{{ $parentIndex }}.title"
                                        ...
                                        <div x-data="{ isOpen: true }" x-init="isOpen = false"
                                            ...
                                            <div x-show="isOpen" class="flex flex-col">
                                                ...
                                                <input type="text" id="nestedCardTitle{{ $nestedIndex }}"
                                                    wire:model="cardContent.{{ $parentIndex }}.data.{{ $nestedIndex }}.cardTitle"
                                                ...
                                                <div class="flex items-center" x-data="{ icon: '' }">
                                                    <div
                                                        class="flex-none bg-gray-100 w-10 px-4 rounded-md text-gray-600 text-center">
                                                        <i x-bind:class="icon" class="w-6 h-6"></i>
                                                    </div>
                                                    <div class="flex-1">
                                                        <input type="text" id="nestedCardIcon{{ $nestedIndex }}"
                                                            wire:model="cardContent.{{ $parentIndex }}.data.{{ $nestedIndex }}.cardIcon"
                                                ...
                                                <textarea id="nestedCardText{{ $nestedIndex }}"
                                                    wire:model="cardContent.{{ $parentIndex }}.data.{{ $nestedIndex }}.cardText"
            ...
            <div class="w-1/4 py-4 px-4">
                ...
                <div class="mt-4">
                    ...
                    <x-input-error :messages="$errors->get('form.type')" for="" class="mt-2" />
                </div>
                <div class="mt-4">
